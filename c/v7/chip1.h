...
class chip1_t {
  ...
  vector<uint8_t> read(vector<uint8_t> scan) {
    gpio_put(MCLK,  0); sleep_16ns();
    gpio_put(SCLK,  0); sleep_16ns();
    gpio_put(SIN,   0); sleep_16ns();
    gpio_put(CLK,   0); sleep_16ns();
    gpio_put(VLD,   0); sleep_16ns();
    gpio_put(CAP,   0); sleep_16ns();
    gpio_put(START, 0); sleep_16ns();
    gpio_put(CLK2,  1); sleep_16ns();

    for (int i=0; i<scan.size(); i++) {
      gpio_put(SIN, scan[i]); sleep_16ns();

      gpio_put(MCLK, 0); sleep_16ns();
      gpio_put(MCLK, 1); sleep_16ns();
      gpio_put(MCLK, 0); sleep_16ns();

      gpio_put(SCLK, 0); sleep_16ns();
      gpio_put(SCLK, 1); sleep_16ns();
      gpio_put(SCLK, 0); sleep_16ns();
    }

    gpio_put(VLD, 1); sleep_16ns();

    gpio_put(CLK, 1); sleep_16ns();
    gpio_put(CLK, 0); sleep_16ns();
    gpio_put(CLK, 1); sleep_16ns();
    gpio_put(CLK2, 0); sleep_16ns();
    gpio_put(CLK2, 1); sleep_16ns();
    gpio_put(CLK, 0); sleep_16ns();
    gpio_put(CLK, 1); sleep_16ns();
    gpio_put(CLK, 0); sleep_16ns();

    gpio_put(CAP, 1); sleep_16ns();
