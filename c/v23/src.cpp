...
uint32_t range(uint32_t tgt, vector<uint8_t> WL, vector<uint8_t> WLB, uint32_t mux, uint32_t sel, uint32_t low, uint32_t high) {
  uint32_t out = 0;
...
uint32_t mix(uint32_t tgt, vector<uint8_t> WL, vector<uint8_t> WLB, uint32_t mux, uint32_t sel) {
  int start = cim4(tgt, WL, WLB, mux, sel);
  int low = max(start - 4, 0);
  int high = min(start + 4, 255);
  uint32_t out = range(tgt, WL, WLB, mux, sel, low, high);
  if (out == 0) {
    return cim3(tgt, WL, WLB, mux, sel);
  }
  return out;
}
...
int main() {
  ...
  while (1) {
    ...
    if (args[0] == 0) {
      ...
      uint32_t word = cim4(args[1], WL, WLB, mux, sel);
      printf("%d\n", word);
    }
    else if (args[0] == 15) {
      sscanf(command, "%u %u %x %x", &args[0], &args[1], &args[2], &args[3]);
      ...
      uint32_t word = mix(args[1], WL, WLB, mux, sel);
      printf("%d\n", word);
    }
    else {
