...
def sample_cim(N1, N2, num_char, size_char, bit, mux):
    measured = []
    expected = []
    expected2 = []

    for n1 in range(N1):
        chars = np.random.choice(a=[0x0, 0x1, 0x2, 0x3], size=(num_char, 8))
        
        array = []
        for i in range(8):
            BL = []
            for j in range(num_char):
                BL.append( int_to_bits( chars[j, i], size_char ) )
            array.append(BL)
        array = np.transpose(array, (1, 2, 0))
        array = np.reshape(array, (num_char * size_char, 8))
        BL = array[:, bit]
        ...
        for n2 in range(N2):
            ...
            WL = char_to_int( match, size_char )
            WLB = (~WL) & ((1 << num_char * size_char) - 1)
            ...
            expected2.append(np.sum( np.array(int_to_bits(WL, 16)) == np.array(BL) ))
    ...
    return (expected, expected2, measured)
...
'''
# avdd_cim, avdd_wl, vbl, vb1, vb0

# 1K
1.0 0 (700, 500, 300, 425, 50)
0.999 1 (700, 500, 325, 425, 50)
1.0 2 (700, 525, 300, 425, 50)
0.999 3 (700, 525, 325, 425, 50)

# 10K
0.9985 0 (700, 500, 300, 425, 50)
0.9984 1 (700, 525, 300, 425, 50)
'''
    ...
    for vbl in [300]:
      for vb1 in [425]:
        for vb0 in [50]:
          ...
          results[(avdd_cim, avdd_wl, vbl, vb1, vb0)] = sample_cim(100, 100, 8, 2, bit, mux)
          np.save('results', results)
