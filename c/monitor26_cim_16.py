...
def sample_cim(N1, N2, num_char, size_char, bit, mux):
    ...
    for n1 in range(N1):
        ...
        for n2 in range(N2):
            num_match_char = np.random.randint(low=0, high=num_char+1)
            num_fp = np.random.randint(low=0, high=num_char-num_match_char+1)
            num_match = 2 * num_match_char + num_fp
            assert num_match <= NWL
            assert num_match >= 0
            ...
            expected2.append( num_match_char )
    ...
    for vbl in [300]:
      for vb1 in [425]:
        for vb0 in [50]:
          ...
          results[(avdd_cim, avdd_wl, vbl, vb1, vb0)] = sample_cim(500, 100, 8, 2, bit, mux)
          np.save('results', results)
